    ...
    ui(new Ui::GroupWidget)
{
    ...
    connect(groupHeader->imGroupAction, SIGNAL(triggered()), this, SLOT(imGroup()));
...
void GroupWidget::set_plain_data(QString plain_data){
   ...
   //qInfo() << plain_group_data_sl.size();
   if ( plain_group_data_sl.at(0) != "" ){
   for (int i = 0; i < plain_group_data_sl.size(); i++){
   ...
   groupHeader->setCounterLabel("(" + QString::number(usersOnlineCount()) + "/" + QString::number(peers.size()) + ")");
...
void GroupWidget::imGroup(){

    emit imGroupSignal(this->group_name);
}
